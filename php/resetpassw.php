...
function validarCorreo() {
    ...
    if (isset($_POST['email'])) {
        ...
        if ($nr == 0) {
            ...
            $user = htmlspecialchars($mostrar['user'], ENT_QUOTES, 'UTF-8');
            $_SESSION['user'] = $user;
            $_SESSION['email'] = htmlspecialchars($correo, ENT_QUOTES, 'UTF-8');
            return true;
        }
    }
    return false;
...
function mostrarErrorYRedirigir() {
    echo "<script>
        alert('Verifique el correo ingresado. O póngase en contacto con el administrador. Registro no existente...');
...
try {
    ...
    if (isset($_POST['signup'])) {
        ...
        $error_encontrado = "";
        $error = true; // Reiniciar variable de error
        ...
        elseif ($password !== $cpassword) {
            ...
            error_log("Contraseñas no coinciden para usuario: " . htmlspecialchars($user, ENT_QUOTES, 'UTF-8'));
        }
        ...
        if ($error) {
            ...
            if ($nr == 1) {
                ...
                if ($stmt_update->execute()) {
                    ...
                    error_log("Contraseña cambiada exitosamente para usuario: " . htmlspecialchars($user, ENT_QUOTES, 'UTF-8'));
                    
                    // Limpiar sesión después de cambio exitoso
                    unset($_SESSION['user']);
                ...
                throw new Exception("No se puede cambiar la contraseña del usuario. Usuario no encontrado.");
            }
        }
    } 
...
<head>
    ...
    <link rel="icon" href="data:image/png;base64,<?php echo htmlspecialchars($icono, ENT_QUOTES, 'UTF-8'); ?>" type="image/png" sizes="32x32">
    ...
    <style>
        ...
        .btn-primary {
            background-color: rgba(219, 206, 90, 1);
            border: 2px solid #6c757d;
            color: white;
        }
...
<body>
    <form id="formreset" role="form" class="containertbl" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post" name="signupform">
        ...
        <div class="logo-container">
            <center>
                <img src="data:image/png;base64,<?php echo htmlspecialchars($logo, ENT_QUOTES, 'UTF-8'); ?>" alt="Logo" width="25%">
            </center><br><br>
        </div>
        ...
        <div class="form-group password-container">
            ...
            <?php if (!empty($password_error)): ?>
                <span class="text-danger"><?php echo htmlspecialchars($password_error, ENT_QUOTES, 'UTF-8'); ?></span>
            <?php endif; ?>
        </div>
        ...
        <div class="form-group password-container">
            ...
            <?php if (!empty($cpassword_error)): ?>
                <span class="text-danger"><?php echo htmlspecialchars($cpassword_error, ENT_QUOTES, 'UTF-8'); ?></span>
            <?php endif; ?>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            setupPasswordToggle('togglePassword', 'floatingPassword');
            setupPasswordToggle('toggleCPassword', 'floatingCPassword');
